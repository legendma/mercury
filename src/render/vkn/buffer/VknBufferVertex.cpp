...
static VKN_buffer_vertex_build_add_sharing_family_proc_type add_sharing_family;
...
static VKN_buffer_vertex_get_proc_type get;
static VKN_buffer_vertex_map_proc_type map;
static VKN_buffer_vertex_build_reset_proc_type reset;
static VKN_buffer_vertex_build_set_allocation_callbacks_proc_type set_allocation_callbacks;
static VKN_buffer_vertex_unmap_proc_type unmap;


/*********************************************************************
*
*   PROCEDURE NAME:
*       VKN_buffer_vertex_create
*
*   DESCRIPTION:
...
Local constants
----------------------------------------------------------*/
static const VKN_buffer_vertex_api_type API =
    {
    get,
    map,
    unmap
...
if( VKN_failed( vkCreateBuffer( buffer->state.logical, &ci_buffer, buffer->state.allocator, &buffer->state.object ) )
 || !buffer->state.memory->i->create_buffer_memory( buffer->state.object, VKN_MEMORY_HEAP_USAGE_DEFAULT, buffer->state.memory, &buffer->state.allocation ) )
    {
    debug_assert_always();
    VKN_buffer_vertex_destroy( NULL, buffer );
    return( FALSE );
    }
...
*
*   PROCEDURE NAME:
*       VKN_buffer_vertex_init_builder
*
*   DESCRIPTION:
*       Initialize a vertex buffer builder.
*
*********************************************************************/
...
Local constants
----------------------------------------------------------*/
static const VKN_buffer_vertex_build_config_type CONFIG =
    {
    add_sharing_family,
    reset,
...
static VKN_BUFFER_VERTEX_CONFIG_API add_sharing_family
    (
    const u32           index,      /* family index to share with   */
...
static VKN_BUFFER_VERTEX_CONFIG_API reset
    (
    struct _VKN_buffer_vertex_build_type
                       *builder     /* vertex buffer builder        */
    )
...
static VKN_BUFFER_VERTEX_CONFIG_API set_allocation_callbacks
    (
