...
#define DEFAULT_PAGE_SIZE           (u32)VKN_size_round_up_mult( 1024 * 1024, builder->state.alloc_alignment )


static VKN_buffer_vertex_dynamic_build_add_sharing_family_proc_type add_sharing_family;
...
static VKN_buffer_vertex_dynamic_clear_proc_type clear;

static VKN_buffer_vertex_dynamic_page_type * create_new_page
    (
    VKN_buffer_vertex_dynamic_type
                       *buffer      /* dynamic vertex buffer        */
    );

static VKN_buffer_vertex_dynamic_flush_proc_type flush;
static VKN_buffer_vertex_dynamic_map_proc_type map;
static VKN_buffer_vertex_dynamic_build_reset_proc_type reset;
static VKN_buffer_vertex_dynamic_build_set_allocation_callbacks_proc_type set_allocation_callbacks;
static VKN_buffer_vertex_dynamic_build_set_page_size_proc_type set_page_size;


/*********************************************************************
*
*   PROCEDURE NAME:
*       VKN_buffer_vertex_dynamic_create
*
*   DESCRIPTION:
...
Local constants
----------------------------------------------------------*/
static const VKN_buffer_vertex_dynamic_api_type API =
    {
    clear,
    flush,
    map
...
buffer->state.alloc_alignment = builder->state.alloc_alignment;
...
*
*   PROCEDURE NAME:
*       VKN_buffer_vertex_dynamic_destroy
*
*   DESCRIPTION:
*       Destroy a dynamic vertex buffer.
*
*********************************************************************/
...
*
*   PROCEDURE NAME:
*       VKN_buffer_vertex_dynamic_init_builder
*
*   DESCRIPTION:
*       Initialize a dynamic vertex buffer builder.
*
*********************************************************************/
...
Local constants
----------------------------------------------------------*/
static const VKN_buffer_vertex_dynamic_build_config_type CONFIG =
    {
    add_sharing_family,
    reset,
...
builder->state.alloc_alignment   = (u16)props->limits.nonCoherentAtomSize;
...
static VKN_BUFFER_VERTEX_DYNAMIC_CONFIG_API add_sharing_family
    (
    const u32           index,      /* family index to share with   */
...
ret   = VKN_arena_allocate_struct( VKN_buffer_vertex_dynamic_page_type, buffer->state.arena );
...
ci_buffer.usage                 = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
...
static VKN_buffer_vertex_dynamic_map_result_type map
    (
...
Local variables
----------------------------------------------------------*/
u32                     alloc_size; /* size in vertex buffer        */
VKN_buffer_vertex_dynamic_map_result_type
                        ret;        /* return mapping               */

alloc_size = (u32)VKN_size_round_up_mult( size, buffer->state.alloc_alignment );
clr_struct( &ret );
debug_assert( alloc_size <= buffer->state.page_size );

/*----------------------------------------------------------
Obtain current page
----------------------------------------------------------*/
if( buffer->state.current
 && buffer->state.current->caret + alloc_size > buffer->state.page_size )
    {
...
Map it
----------------------------------------------------------*/
debug_assert( buffer->state.current->caret % buffer->state.alloc_alignment == 0 );
ret.offset  = buffer->state.current->caret;
ret.buffer  = buffer->state.current->buffer;
ret.mapping = &buffer->state.current->allocation.mapping[ ret.offset ];

buffer->state.current->caret      += alloc_size;
buffer->state.current->dirty_size += alloc_size;
...
static VKN_BUFFER_VERTEX_DYNAMIC_CONFIG_API reset
    (
    struct _VKN_buffer_vertex_dynamic_build_type
                       *builder     /* dynamic vertex buffer builder*/
    )
...
static VKN_BUFFER_VERTEX_DYNAMIC_CONFIG_API set_allocation_callbacks
    (
...
static VKN_BUFFER_VERTEX_DYNAMIC_CONFIG_API set_page_size
    (
...
Local literals
----------------------------------------------------------*/
#define MIN_PAGE_SIZE               (u32)VKN_size_round_up_mult( 1024, builder->state.alloc_alignment )
...
aligned_page_size = (u32)VKN_size_round_up_mult( page_size, builder->state.alloc_alignment );
if( aligned_page_size < MIN_PAGE_SIZE )
    {
